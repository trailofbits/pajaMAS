...
def run_agent_with_prompt(url, httpd, server_thread):
    ...
    session = session_service.create_session(app_name="simple_mas", user_id="test_user")
...
def main():
    """Set up a local HTTP server for test.html, run the MAS agent, and clean up."""
    ...
    parser.add_argument(
        ...
        type=str,
        default="test.html",
        help="HTML file to serve (default: test.html)",
    )
    parser.add_argument(
        "--port",
        type=int,
